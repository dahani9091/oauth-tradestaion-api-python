...
class TradeStationClient():
    ...
    def __init__(self, username: str, client_id: str, client_secret: str, paper_trading: bool = True, redirect_uri: str = 'http://localhost:3000') -> None:
        ...
        ----
            ...
            >>> tradestation_client
        """
        TradeStationClient.REDIRECT_URI = redirect_uri
        TradeStationClient.API_KEY = client_id
    ...
    def _initialize_server(self):
        ...
        TradeStationClient.HTTPD = httpd
        print(f'Serving on {TradeStationClient.REDIRECT_URI}')
        webbrowser.open(TradeStationClient.REDIRECT_URI)
        httpd.serve_forever()
        TradeStationClient.HTTPD = None

    @staticmethod
    def getAccessUrl():

        query_string = urllib.parse.urlencode({
            'redirect_uri': TradeStationClient.REDIRECT_URI,
            'client_id': TradeStationClient.API_KEY,
            'response_type': 'code',
    ...
    def _grab_access_token(self) -> bool:
        ...
        # Parse the URL
        if not TradeStationClient.REDIRECT_URI.endswith('/'):
            TradeStationClient.REDIRECT_URI = TradeStationClient.REDIRECT_URI + '/'
        url_code_format = TradeStationClient.REDIRECT_URI + f"?code={self.state['redirect_code']}"
        url_dict = urllib.parse.parse_qs(url_code_format)
    ...
    def _authorize(self) -> None:
        ...
        """
        threading.Thread(target=self._initialize_server).start()
        cur_dir = os.path.dirname(__file__)
        path = os.path.join(cur_dir, 'temp','code.txt')
        while True:
            try:
                if os.path.exists(path):
                    with open(path, 'r') as f:
                        self.state['redirect_code'] = f.read().strip()
                        print(self.state['redirect_code'])
                    # delete the file
                    os.remove(path)
                    break
                print("Sleeping")
